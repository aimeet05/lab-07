...
*The histogram is skewed to the left. Meaning that a higher number of students rated their courses highly. This is expected as the students are likely to pick courses they will enjoy, so rank high.*
...
*A positive correlation between score and bty_avg. Jitter moves points slights to help give a better representation of the data, specifically where you have clustered of points which would be hard to see with discreet data.*
...
_Professors with a high bty_avg will have a higher score by 0.066*bty_avg_
...
*Professors with a low bty_avg (example = 0) will have a minimum of 3.8 score*
...
ggplot(data = score_bty_aug,
       mapping = aes( x = .fitted,
                      y = .resid)) + geom_jitter() + geom_hline(linetype = "dashed", yintercept = 0)
